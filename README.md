...
- [ ] **数据库优化**
  - [ ] 添加消息表的 `room_id` 和 `created_at` 联合索引优化
  - [ ] 实现消息自动清理机制（保留最近N天，默认30天，可通过环境变量配置）
  - [ ] 添加数据库连接池配置（SQLite WAL模式，提升并发性能）
  - [ ] 优化查询语句，减少不必要的数据传输（只查询需要的字段）
  - [ ] 添加数据库迁移脚本，确保向后兼容

- [ ] **内存管理**
  - [ ] 实现房间和用户列表的定期清理（断开连接后延迟5分钟删除，避免频繁创建）
  - [ ] 添加内存使用监控和告警（超过80%内存使用率时告警）
  - [ ] 优化 Socket.IO 连接管理（心跳检测优化）
  - [ ] 实现消息频率限制的数据结构优化（使用Map而非Array，提升查找效率）

- [ ] **代码优化**
  - [ ] 重构消息频率限制逻辑，使用更高效的数据结构（已完成，检查是否需要进一步优化）
  - [ ] 添加请求日志（可选，生产环境可关闭，通过环境变量控制）
  - [ ] 优化错误处理机制（统一错误格式，添加错误码）
  - [ ] 代码注释完善（关键函数添加JSDoc注释）

- [ ] **监控与健康检查**
  - [ ] 增强 `/health` 端点，返回内存、连接数、数据库大小等指标
  - [ ] 添加性能监控中间件（可选，记录慢请求）
  - [ ] 实现健康检查的定期自检（每5分钟检查一次数据库连接）
...
- [ ] **房间功能增强**
  - [ ] 为 `rooms` 表添加 `name` 字段（房间名称，可选）
  - [ ] 为 `rooms` 表添加 `description` 字段（房间描述，可选）
  - [ ] 为 `rooms` 表添加 `password` 字段（房间密码，NULL表示开放房间）
  - [ ] 为 `rooms` 表添加 `creator_session` 字段（创建者session_id，用于权限判断）
  - [ ] 实现数据库迁移脚本（向后兼容，使用ALTER TABLE ADD COLUMN IF NOT EXISTS）
  - [ ] 前端支持显示和编辑房间名称（侧边栏添加编辑按钮）
  - [ ] 前端支持设置房间密码（创建者功能，密码长度限制4-20字符）
  - [ ] 前端支持加入密码房间（输入密码界面，密码错误提示）
  - [ ] 实现密码验证逻辑（join_room时验证，密码错误返回明确错误信息）
  - [ ] 实现修改密码功能（创建者可修改，修改时清空聊天记录并通知所有用户）
  - [ ] 实现取消密码功能（创建者可以移除密码，变为开放房间）
  - [ ] 限制默认房间（"1"）不能设置密码（前端和后端双重验证）
  - [ ] 添加房间信息显示（显示房间是否有密码、创建者信息等）
  - [ ] 房间列表页面（可选功能）

- [ ] **消息类型支持**
  - [ ] 为 `messages` 表添加 `content_type` 字段（text/emoji/file等，默认'text'）
  - [ ] 实现消息类型自动检测逻辑（检测Emoji、URL等）
  - [ ] 支持 Emoji 表情消息（支持Unicode Emoji，如 😀 🎉 ✅ 等）
  - [ ] 前端渲染不同类型的消息（不同消息类型使用不同样式）
  - [ ] 添加消息类型图标（在消息前显示类型图标）

- [ ] **用户体验优化**
  - [ ] 消息时间显示优化（相对时间：刚刚、1分钟前等）
  - [ ] 添加消息发送状态指示（发送中、已发送）
  - [ ] 优化移动端响应式布局
  - [ ] 添加键盘快捷键（Enter发送，Shift+Enter换行）
