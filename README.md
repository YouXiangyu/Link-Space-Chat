...
- **自动创建**: 您**无需**手动创建数据库。首次启动服务时，`db/index.js` 会自动初始化数据库结构并执行迁移，创建所需的数据表（`rooms` 和 `messages`）及索引。
- **文件忽略**: `chat.db` 文件包含了所有的聊天历史记录。为了避免将个人或测试的聊天数据提交到版本控制系统（如 Git），`.gitignore` 文件已配置为忽略 `chat.db`。这确保了仓库的干净，同时保护了本地数据的私密性。

### 静态资源说明

- 现行界面仅依赖 `public/cyberStyles.css` 与 `public/lightStyles.css`。
- 历史样式文件已归档至 `docs/legacy-css/`，不会随静态资源部署，如需参考可在该目录查阅。
